...
OUTPUT_DIR = CallTimePath("ORACLE_OUTPUT_DIR", lambda: _oracle_work_root() / "stage1_oracle_backends" / "alpha")
CONFIG_PATH = CallTimePath("ORACLE_CONFIG", lambda: _project_root() / "contracts" / "oracle_components" / "alpha" / "stage4_alpha_config.yaml")
...
for vid in SEL_IDS:
    ...
    rho_raw = DIRS[vid].get('spearman_rho')
    rho = float(rho_raw) if rho_raw is not None else float('nan')
...
nested_selection_path = INPUT_DIR / "stage3_v2" / "nested_development_contract.json"
if not nested_selection_path.exists():
    raise FileNotFoundError(f"Missing Stage00_04 nested-development contract: {nested_selection_path}")
with open(nested_selection_path, encoding="utf-8") as f:
    nested_selection_contract = json.load(f)
expected_selection = f"{DEV_YEARS[0]}-{INNER_TRAIN_END}"
expected_validation = f"{INNER_VAL_START}-{INNER_VAL_END}"
if nested_selection_contract.get("selection_inner_train") != expected_selection:
    raise ValueError("Stage00_04 selection split does not match Alpha inner-train split")
if nested_selection_contract.get("inner_validation_evaluation_only") != expected_validation:
    raise ValueError("Stage00_04 inner-validation split does not match Alpha")


def fit_inner_train_representation(frame, fit_mask, output_mask):
    """Fit winsor/bin/isotonic/imputation on inner train and transform DEV.

    No 2017-2019 label or distribution statistic is read while constructing
    this representation. The existing Phase 4-7 representation remains the
    separate full-DEV final-refit representation.
    """
    fit_mask = pd.Series(fit_mask, index=frame.index).astype(bool)
    output_mask = pd.Series(output_mask, index=frame.index).astype(bool)
    fit_rating = pd.to_numeric(frame.loc[fit_mask, 'rating_num'], errors='coerce')
    fit_r_min, fit_r_max = float(fit_rating.min()), float(fit_rating.max())
    if not np.isfinite(fit_r_min) or not np.isfinite(fit_r_max) or fit_r_max <= fit_r_min:
        raise ValueError("Inner-train rating range is not estimable")

    score_columns = {}
    artifact = {
        'contract_version': 'alpha_inner_train_representation_v1',
        'fit_years': f'{DEV_YEARS[0]}-{INNER_TRAIN_END}',
        'validation_years': f'{INNER_VAL_START}-{INNER_VAL_END}',
        'fit_rows': int(fit_mask.sum()),
        'validation_labels_used_for_representation': False,
        'validation_distribution_used_to_refit_block_normalization': False,
        'variables': {},
    }
    for vid in SEL_IDS:
        raw = pd.to_numeric(frame[vid], errors='coerce')
        fit_values = raw[fit_mask].dropna()
        if fit_values.empty:
            raise ValueError(f"Inner-train representation has no observations for {vid}")
        n_unique = int(fit_values.nunique())
        is_binary = n_unique <= 2
        if is_binary:
            lower = upper = None
            transformed = raw.copy()
        else:
            lower, upper = np.percentile(fit_values, [1, 99])
            transformed = raw.clip(lower, upper)
        fit_transformed = transformed[fit_mask].dropna()
        unique_values = sorted(float(v) for v in fit_transformed.unique())
        if is_binary or n_unique <= LOW_UNIQUE_THRESHOLD:
            edges = unique_values
            method = 'unique_value' if is_binary else 'reduced_unique_value'
            n_bins = len(edges)
            def assign_bin(x, uv=edges):
                return min(range(len(uv)), key=lambda i: abs(float(x) - uv[i])) if pd.notna(x) else np.nan
        else:
            edges_np = np.linspace(float(fit_transformed.min()), float(fit_transformed.max()), 51)
            edges = [float(x) for x in edges_np]
            method, n_bins = 'equal_width_50', 50
            def assign_bin(x, e=edges_np):
                return max(0, min(int(np.searchsorted(e, float(x), side='right')) - 1, len(e) - 2)) if pd.notna(x) else np.nan
        bins = transformed.apply(assign_bin)

        bins_data = []
        for b in range(n_bins):
            row_mask = fit_mask & bins.eq(b)
            if bool(row_mask.any()):
                mean_rating = pd.to_numeric(frame.loc[row_mask, 'rating_num'], errors='coerce').mean()
                empirical = 100.0 * (fit_r_max - mean_rating) / (fit_r_max - fit_r_min)
                bins_data.append({'raw_bin': b, 'emp': empirical, 'count': int(row_mask.sum())})
        if not bins_data:
            raise ValueError(f"Inner-train isotonic table is empty for {vid}")
        bdf = pd.DataFrame(bins_data)
        direction = DIRS[vid]['direction']
        bdf['gd'] = ((n_bins - 1) - bdf['raw_bin']) if direction == 'value_down_good' else bdf['raw_bin']
        bdf = bdf.sort_values('gd')
        x = bdf['gd'].to_numpy(dtype=float)
        y = bdf['emp'].to_numpy(dtype=float)
        weights = bdf['count'].to_numpy(dtype=float)
        if len(x) >= 2:
            fitted = IsotonicRegression(increasing=True, out_of_bounds='clip').fit_transform(x, y, sample_weight=weights)
        else:
            fitted = y.copy()
        fmin, fmax = float(fitted.min()), float(fitted.max())
        fitted = ((fitted - fmin) / (fmax - fmin) * 100.0) if fmax > fmin else np.full_like(fitted, 50.0)
        score_map = {int(row.raw_bin): float(np.clip(fitted[i], 0, 100)) for i, row in enumerate(bdf.itertuples(index=False))}
        scores = bins.map(lambda b, sm=score_map: sm.get(int(b), np.nan) if pd.notna(b) else np.nan)
        imputation = float(scores[fit_mask].median())
        if not np.isfinite(imputation):
            raise ValueError(f"Inner-train imputation score is not finite for {vid}")
        score_columns[vid] = scores.fillna(imputation)
        artifact['variables'][vid] = {
            'winsor_p01': None if lower is None else float(lower),
            'winsor_p99': None if upper is None else float(upper),
            'binning_method': method,
            'bin_edges': edges,
            'isotonic_score_map': {str(k): v for k, v in score_map.items()},
            'imputation_score': imputation,
        }
    scores_df = pd.DataFrame(score_columns, index=frame.index)
    fin = scores_df.loc[output_mask, FIN_IDS].to_numpy(dtype=float)
    nonfin = scores_df.loc[output_mask, NONFIN_IDS].to_numpy(dtype=float)
    return fin, nonfin, artifact


inner_fit_mask_full = dev_mask & (pd.to_numeric(df['year'], errors='coerce') <= INNER_TRAIN_END)
inner_dev_fin_scores, inner_dev_nf_scores, inner_representation_artifact = fit_inner_train_representation(
    df, inner_fit_mask_full, dev_mask
)
with open(OUTPUT_DIR / 'inner_representation_contract_alpha.json', 'w', encoding='utf-8') as f:
    json.dump(inner_representation_artifact, f, indent=2, ensure_ascii=False)
...
def r_score_from_weights(fin_scores, nf_scores, w_fin, w_nf, dev_idx=None, norm_bounds=None):
    fin_raw = fin_scores @ w_fin
    nf_raw = nf_scores @ w_nf
    if norm_bounds is not None:
        fp01 = float(norm_bounds['financial']['p01'])
        fp99 = float(norm_bounds['financial']['p99'])
        np01 = float(norm_bounds['nonfinancial']['p01'])
        np99 = float(norm_bounds['nonfinancial']['p99'])
...
train_fin = inner_dev_fin_scores[inner_train_mask]
train_nf = inner_dev_nf_scores[inner_train_mask]
train_actual = dev_actual[inner_train_mask]
train_rating_num = dev_rating_num[inner_train_mask]

val_fin = inner_dev_fin_scores[inner_val_mask]
val_nf = inner_dev_nf_scores[inner_val_mask]
...
for a_em in grid_alpha_em:
    for b_kl in grid_beta_kl:
        for g_reg in grid_gamma_reg:
            ...
            # Score validation with inner-train block-normalization bounds.
            # Validation feature distribution must not refit the representation.
            _, inner_train_block_norm = r_score_from_weights(train_fin, train_nf, w_fin, w_nf)
            R_val, _ = r_score_from_weights(
                val_fin, val_nf, w_fin, w_nf, norm_bounds=inner_train_block_norm
            )
...
params = {
    ...
    'inner_cv_split': {
        ...
        'variable_selection_fit': nested_selection_contract.get('selection_inner_train'),
        'representation_fit': f'{DEV_YEARS[0]}-{INNER_TRAIN_END}',
        'validation_labels_used_for_representation': False,
        'validation_distribution_used_to_refit_block_normalization': False,
        'final_representation_refit': f'{DEV_YEARS[0]}-{DEV_YEARS[1]}'
    },
    ...
    'hyperparameter_selection': 'Nested inner validation: variable selection + representation fit 2002-2016; evaluation-only 2017-2019; final refit 2002-2019',
