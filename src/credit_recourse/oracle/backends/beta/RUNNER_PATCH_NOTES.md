...
- Writes outputs to project-level `archive/DEPLOYED_RELEASE/stage01_beta_ordered_logit` instead of bundle-local `outputs`.
...
```powershell
cd .
.\src\credit_recourse\oracle\pipelines\stage01_oracle_construction\beta\scripts\run_stage4_beta.ps1 -ForceRefreshInputs
```

If the script cannot infer the repo root:

```powershell
.\src\credit_recourse\oracle\pipelines\stage01_oracle_construction\beta\scripts\run_stage4_beta.ps1 `
  -ProjectRoot . `
  -ForceRefreshInputs
```
