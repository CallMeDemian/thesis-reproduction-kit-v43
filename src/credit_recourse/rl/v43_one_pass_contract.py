...
def freeze_run_config(root, encoder_contract):
    ...
    payload = {'version':'V43OnePassRun/4','temporal':TEMPORAL.to_dict(),
        ...
        'seed':2,'stages':stages,'architecture':encoder_contract.to_dict()['architecture'],
    ...
    assert all(stages[s]['seed']==payload['seed'] for s in ('stage3','stage4','stage5'))
    payload['config_hash'] = content_hash(payload)
    destination = root/Path(run_path(root))/'01_contract/resolved_run_config.json'
...
def load_run_config(root):
    path = Path(root)/Path(run_path(root))/'01_contract/resolved_run_config.json'
