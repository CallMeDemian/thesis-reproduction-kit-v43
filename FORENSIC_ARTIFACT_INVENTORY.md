...
| Oracle deployment family | FOUND | `archive\\DEPLOYED_RELEASE\\stage0_oracle_foundation`, `stage1_oracle_inputs`, `stage1_oracle_backends` | Deployment total 1,436 files / 5,072,088,366 bytes. Stage0: 7 files / 124,411,620 bytes. Stage1 inputs: 170 files / 178,749,154 bytes. | **Migrated and hash-verified** at `frozen/original_release/oracle/` using Git LFS; 234/234 selected Oracle files match. |
...
| RL C3-E final release | FOUND | `archive\\DEPLOYED_RELEASE\\stage5_candidate_iql\\C3E_E2_7SEED_BALANCED_DFEBAFA6` | 205 files / 475,155,224 bytes; exactly 28 `.pt` actor checkpoints / 470,944,432 bytes. Four configurations × seven seeds are present. | **Migrated and hash-verified** at `frozen/original_release/rl/C3E_E2_7SEED_BALANCED_DFEBAFA6` using Git LFS; 205/205 files match. |
...
| Stage8 evaluation family | FOUND | `data\\reproduction\\llm_final_evaluation_20260913` | 240 stage8/stage9-named paths, 515,657,017 bytes in the source tree. Noncanonical all-firm result: 184,803 rows, 16,260,714 bytes, SHA-256 `32d5dc319256263a133ad02da502b6e3f07a5b69780fee29f844c2bc1b0a6eae`. Canonical frozen Stage8 is the 96,600-row artifact above. | **Selected 161-file evaluation pack migrated and hash-verified**; canonical frozen Stage8 remains the authority and variants stay explicitly labeled. |
| Stage9 comparison / revision outputs | FOUND | `data\\reproduction\\llm_final_evaluation_20260913\\stage9` | 76 files / 320,187,946 bytes. Four explicit arms exist: `BASELINE_STRICT_ITT`, `BASELINE_REPAIRED_ITT`, `HIGH_STRICT_ITT`, `HIGH_REPAIRED_ITT`. Primary comparison Parquet row counts are 52,824; 60,931; 60,700; and 60,948 respectively. | **Migrated and hash-verified** in the Stage8/9 evaluation pack; canonical/diagnostic labeling remains explicit. |
...
1. Keep the live-call gate closed unless a fresh replication is explicitly
   approved; historical artifacts must never be used as an implicit live-call
   authorization.
2. Keep `provenance/MIGRATION_MANIFEST.json` and
   `provenance/ASSET_PACK_MANIFEST.json` synchronized whenever a release asset
   changes.
3. Resolve the remaining source commit-object gaps externally if exact Git
   provenance is required; the scientific artifact packs themselves are now
   hash-verified in the new repository.
4. Preserve the distinction between certified frozen replay and fresh runs;
   migrated original assets do not turn a fresh run into a historical replay.
